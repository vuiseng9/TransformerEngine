...
__host__ __device__ inline float2 fp4x2byte_to_float2(uint8_t packed_e2m1) {
    __nv_fp4x2_storage_t packed;
    // aliasing-safe copy of the byte into the storage type
    memcpy(&packed, &packed_e2m1, sizeof(packed));

    __half2_raw h2r = __nv_cvt_fp4x2_to_halfraw2(packed, __NV_E2M1);
    __half2 h2;
    memcpy(&h2, &h2r, sizeof(h2));           // avoid reinterpret_cast aliasing
...
dequantize_mxnv_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
  ...
  constexpr size_t THREADS_PER_CHUNK_X_ROWWISE = (CHUNK_DIM_X / packing) / ELEMS_PER_THREAD;
  constexpr size_t THREADS_PER_CHUNK_X_COLWISE = CHUNK_DIM_X / packing;                     
  constexpr size_t ELEMS_PER_THREAD = 16 / packing;  // (mx:16, nv:8)
  ...
  constexpr size_t THREADS_PER_SCALE_X_ROWWISE =
      DIVUP(SCALE_DIM_X, (ELEMS_PER_THREAD*packing));  //  mx:2, nv=1 why *packing? because 2 logical element per unit physical elements
  ...
  for (int iter = 0; iter < ITERATIONS; ++iter) {
    ...
    const float block_scale = [&] {
      if constexpr (std::is_same_v<ScaleType, e8m0_t>) {
        const ScaleType biased_exponent = scales_ptr[scale_idx];
    ...
    if constexpr (USE_ROWWISE_SCALING) {
      ...
      if constexpr (std::is_same_v<IType, fp4e2m1>) {
        for (int j = 0; j < ELEMS_PER_THREAD; ++j) {
          float2 f2 = fp4x2byte_to_float2(static_cast<uint8_t>(in.data.elt[j]));
          out.data.elt[j*2]   = static_cast<OType>(block_scale * f2.x);
          out.data.elt[j*2+1] = static_cast<OType>(block_scale * f2.y);
        }
      } else {
#pragma unroll
        for (int j = 0; j < ELEMS_PER_THREAD; ++j) {
      ...
      if constexpr (std::is_same_v<IType, fp4e2m1>) {
        for (int i = 0; i < BUFFER_DIM_Y; ++i) {
          float2 f2 = fp4x2byte_to_float2(static_cast<uint8_t>(in_sh[buff][i][tid_colwise_X]));
          out_sh[buff][i*2  ][tid_colwise_X] = static_cast<OType>(block_scale * f2.x);
          out_sh[buff][i*2+1][tid_colwise_X] = static_cast<OType>(block_scale * f2.y);
        }
      } else {
#pragma unroll
        for (int i = 0; i < BUFFER_DIM_Y; ++i) {
...
static void mxnv_dequantize(const Tensor &input, Tensor *output, cudaStream_t stream) {
  ...
  if (use_rowwise_scaling) {
    ...
    NVTE_CHECK(is_narrow_dtype(input.data.dtype), "Input must have FP8/FP4 type.");
  }

  if (use_colwise_scaling) {
    NVTE_CHECK(input.has_columnwise_data(), "Cannot dequantize tensor without columnwise data.");
    NVTE_CHECK(is_narrow_dtype(input.columnwise_data.dtype), "Input must have FP8/FP4 type.");
  }

  NVTE_CHECK(!is_narrow_dtype(output->data.dtype), "Output must be in higher precision.");
  // Output Shape has been validated in caller
  // NVTE_CHECK(output->data.shape == input.data.shape, "Input and output shapes need to match.");
